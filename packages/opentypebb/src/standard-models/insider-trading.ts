...
export const InsiderTradingQueryParamsSchema = z.object({
  ...
  transaction_type: z.string().nullable().default(null).describe('Type of transaction: Purchase, Sale, Award, etc.'),
  insider_type: z.string().nullable().default(null).describe('Type of insider: director, officer, 10_percent_owner.'),
  start_date: z.string().nullable().default(null).describe('Start date (YYYY-MM-DD).'),
  end_date: z.string().nullable().default(null).describe('End date (YYYY-MM-DD).'),
}).passthrough()
...
export const InsiderTradingDataSchema = z.object({
  ...
  transaction_code: z.string().nullable().default(null).describe('SEC transaction code (P=Purchase, S=Sale, A=Award, etc.).'),
  transaction_value: z.number().nullable().default(null).describe('Total transaction value in USD.'),
  pre_transaction_shares: z.number().nullable().default(null).describe('Shares owned before the transaction.'),
  remaining_shares: z.number().nullable().default(null).describe('Shares remaining after the transaction.'),
}).passthrough()
